	...
	$.ajax({
				...
				success : function(map) {
					if (map.user_iswrite == "F" || map.user_iswrite == null)
						...
						$("#sketchForm").modal();
						var htmlModal = "<input type='hidden' name='user_email' value='"
								+ map.user_email
								+ "'>"
								+ "<div class='form-group'>"
								+ "<label>스케치북 제목</label>"
								+ "<div class='modal-input'>"
								+ "<input type='text' class='form-control' id='sketchtitle' name='sketch_title' required='required'></div>"
								+ "</div>"
								+ "<div class='form-group'>"
								+ "<label>스케치북 테마</label>"
...
function fileUpload() {
	...
	$.ajax({
		...
		success : function(result) {
			alert("아작스 결과" + result);
			$("div[id=modalIMG1]").css("background-image",
					"url('" + result + "')");
