    ...
    // 일정 마커들의 인포윈도우
     ...
     // 일정 마커들의 주소
      ...
      var infowindow = new daum.maps.InfoWindow({
           content: '<div><%=daysList.get(j).getDays_title()%></div>', // 인포윈도우에 표시할 내용
           removable : true
      });
      // 시작시간 , 종료시간 
      ...
      var daysPage = document.getElementById("page");          
         ...
         for(var i = 0 ; i < daysMarker.length ; i++){
            ...
            // 시작시간 
            var startTime =  daysStart[i].split(":");
            var startHalf = "AM";
            if(parseInt(startTime[0])>= 12){
               startTime[0] = parseInt(startTime[0])-12;
               startHalf = "PM";
            }
            if(parseInt(startTime[0]) < 10) { startTime[0] = "0"+ parseInt(startTime[0]); }
            if(parseInt(startTime[1]) < 10) { startTime[1] = "0"+ parseInt(startTime[1]); }
            
            var endTime = daysEnd[i].split(":");
            var endHalf = "AM";
