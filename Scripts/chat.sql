		...
		SELECT SKETCH_ID, SKETCH_THEME, SKETCH_TITLE, SKETCH_SPATH, SKETCH_SHARE, SKETCH_BLOCK
			FROM (SELECT ROWNUM RNUM, SKETCH_ID, SKETCH_THEME, SKETCH_TITLE, SKETCH_SPATH, SKETCH_SHARE, SKETCH_BLOCK
				FROM (SELECT SKETCH_ID, SKETCH_THEME, SKETCH_TITLE, SKETCH_SPATH, SKETCH_SHARE, SKETCH_BLOCK
					FROM LPSKETCHBOOK WHERE USER_EMAIL='user1@email.com' ORDER BY TO_NUMBER(SKETCH_ID) DESC)) WHERE RNUM BETWEEN 1 AND 9
...
FROM (SELECT ROWNUM RNUM, SKETCH_ID, SKETCH_THEME, SKETCH_TITLE, SKETCH_SPATH
	FROM (SELECT LPSKETCHBOOK.SKETCH_ID, SKETCH_THEME, SKETCH_TITLE, SKETCH_SPATH FROM LPSKETCHBOOK, LPCOLLECT 
		WHERE LPCOLLECT.USER_EMAIL='user1@email.com'
...
ORDER BY SKETCH_ID DESC)) WHERE RNUM BETWEEN  1 AND 9;




SELECT LPSKETCHBOOK.SKETCH_ID, SKETCH_THEME, SKETCH_TITLE, SKETCH_SPATH, LPCOLLECT.USER_EMAIL FROM LPSKETCHBOOK, LPCOLLECT 
		WHERE LPSKETCHBOOK.SKETCH_ID = LPCOLLECT.SKETCH_ID AND LPCOLLECT.USER_EMAIL = 'user1@email.com';
...
-- 내가 SENDER인지 RECEIVER인지 판단이 필요할 듯함 
...
CREATE TABLE LPCHATIMAGE(CHI_ID VARCHAR2(20), CHR_ID VARCHAR2(20), CHI_RPATH VARCHAR2(200), CHI_SPATH VARCHAR2(200), FILE_SIZE NUMBER);
...
-- 사용자가 닉네임 수정시 LPCHATLIST도 수정
UPDATE LPCHATLIST SET USER_NICKNAME=${'변경된 닉네임'} WHERE USER_NICKNAME=${'이전 닉네임'}